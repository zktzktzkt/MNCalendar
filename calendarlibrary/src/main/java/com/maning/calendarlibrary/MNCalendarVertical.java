...
public class MNCalendarVertical extends LinearLayout {
    ...
    private void initAdapter() {
        if (mnCalendarVerticalAdapter == null) {
            ...
            if (!TextUtils.isEmpty(mnCalendarVerticalConfig.getMnCalendar_startDate())
                    && !TextUtils.isEmpty(mnCalendarVerticalConfig.getMnCalendar_endDate())) {

                mnCalendarVerticalAdapter.updateDatas(dataMap, currentCalendar, mnCalendarVerticalConfig,
                                                      getDate(mnCalendarVerticalConfig.getMnCalendar_startDate()),
                                                      getDate(mnCalendarVerticalConfig.getMnCalendar_endDate()));
            } else {
                mnCalendarVerticalAdapter.updateDatas(dataMap, currentCalendar, mnCalendarVerticalConfig);
            }
        }
    ...
    private Date getDate(String dateStr) {
        ...
        label:
        for (Map.Entry<String, ArrayList<MNCalendarItemModel>> entry : dataMap.entrySet()) {
            for (int i = 0; i < entry.getValue().size(); i++) {
                if (dateStr.equals(MNConst.sdf_yyy_MM_dd.format(entry.getValue().get(i).getDate()))) {
                    Log.e("时间相等 开始", MNConst.sdf_yyy_MM_dd.format(entry.getValue().get(i).getDate()));
                    date = entry.getValue().get(i).getDate();
                    break label;
                }
            }
        }
